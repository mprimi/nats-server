...
func newChainOfBlocksStateMachine(name string, s *Server, observer *chainOfBlocksObserver) (*ChainOfBlocksStateMachine, error) {
	...
	fs, err := newFileStoreWithCreated(
		FileStoreConfig{StoreDir: storeDir, BlockSize: defaultMediumBlockSize, AsyncFlush: false, SyncInterval: 10 * time.Second, srv: s},
	...
	cfg := &RaftConfig{Name: name, Store: storeDir, Log: fs, Track: true}
	...
	labels := pprofLabels{}
	n, err := s.startRaftNode(globalAccountName, cfg, labels)
	if err != nil {
		return nil, err
	}

	if bootstrap || n.Term() == 0 {
		err := n.Campaign()
		if err != nil {
			return nil, err
		}
